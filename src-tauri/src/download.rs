...
const DIAGNOSTIC_LIMIT_BYTES: usize = 128 * 1024;
...
pub struct DownloadRequest {
    ...
    pub selected_format_has_audio: Option<bool>,
    #[serde(default)]
    pub compatibility_mode: bool,
    #[serde(default)]
    pub access_strategy: Option<YoutubeAccessStrategy>,
    #[serde(default)]
...
fn valid_container(value: &str) -> bool {
    matches!(value, "auto" | "mp4" | "mkv" | "webm")
}
...
fn valid_browser_session(value: Option<&str>) -> bool {
    value.is_none_or(|browser| BrowserSession::parse(browser).is_some())
}
...
impl DownloadRequest {
    fn effective_access_strategy(&self) -> Result<YoutubeAccessStrategy, String> {
        ...
        let browser = match self.browser_session.as_deref() {
            Some(value) => Some(
                BrowserSession::parse(value)
                    .ok_or_else(|| "El navegador seleccionado no es compatible.".to_owned())?,
            ),
            None => None,
        };
        Ok(match browser {
    ...
    fn set_access_strategy(&mut self, strategy: YoutubeAccessStrategy) {
        self.browser_session = strategy
            .browser()
            .map(|browser| browser.as_str().to_owned());
        self.use_pot_provider = strategy.uses_pot_provider();
        self.access_strategy = Some(strategy);
    }
}
...
fn paths_ready(app: &AppHandle) -> Result<engine::EnginePaths, String> {
...
fn emit_queue(app: &AppHandle) {
    let _ = app.emit("queue://updated", snapshot(app));
}
...
fn update_job<F>(app: &AppHandle, job_id: &str, update: F) -> Option<DownloadJob>
where
...
pub fn add(app: AppHandle, mut request: DownloadRequest) -> Result<DownloadJob, String> {
    paths_ready(&app)?;
    if !valid_container(&request.container) {
        return Err("Contenedor no válido.".to_owned());
    }
    if request
        .selected_format_id
        .as_deref()
        .is_some_and(|format_id| !valid_format_id(format_id))
    {
        return Err("El formato seleccionado no es válido.".to_owned());
    }
    if !valid_browser_session(request.browser_session.as_deref()) {
        return Err("El navegador seleccionado no es compatible.".to_owned());
    }
    let strategy = request.effective_access_strategy()?;
    request.set_access_strategy(strategy);
    if request.video_id.trim().is_empty()
        || request.url.trim().is_empty()
        || request.destination.trim().is_empty()
    {
        return Err("Faltan datos para agregar la descarga a la cola.".to_owned());
    }
    if !valid_youtube_url(&request.url) {
        return Err("La URL no es un enlace de YouTube válido.".to_owned());
    }
    if app
        .state::<DownloadManager>()
        .state
        .lock()
        .map_err(|_| "No se pudo bloquear la cola.".to_owned())?
        .jobs
        .iter()
        .any(|existing| {
            existing.request.video_id == request.video_id
                && !matches!(
                    existing.status,
    ...
    let job = DownloadJob {
        ...
        message: Some("En espera".to_owned()),
        error: None,
        file_path: None,
...
pub fn get_queue(app: AppHandle) -> QueueSnapshot {
    snapshot(&app)
}
...
pub fn start(app: AppHandle) -> Result<(), String> {
    ...
    emit_queue(&app);
    if should_start {
        thread::spawn(move || run_queue(app));
    }
    Ok(())
}
...
fn next_job(app: &AppHandle) -> Option<DownloadJob> {
    ...
    let Some(job) = state
        .jobs
        .iter_mut()
        .find(|job| matches!(job.status, DownloadStatus::Queued | DownloadStatus::Pending))
    else {
        state.worker_running = false;
        return None;
    };
    ...
    if let Err(error) = execute_download(app, job) {
        ...
        if !cancelled {
            ...
            update_job(app, &job.job_id, |current| {
                ...
                current.message = Some("La descarga falló".to_owned());
            });
            emit_job(
                app,
                "download://failed",
...
fn parse_bytes(value: Option<&str>) -> Option<u64> {
...
fn parse_progress(line: &str) -> Option<DownloadProgress> {
    ...
    Some(DownloadProgress {
        percent,
        downloaded_bytes: parse_bytes(fields.get(2).copied()).unwrap_or(0),
        total_bytes: total,
        speed: parse_number(fields.get(5).copied()),
        eta: parse_number(fields.get(6).copied()),
    })
}
fn dispatch_line(
    app: &AppHandle,
    job_id: &str,
    line: &str,
    final_path: &Arc<Mutex<Option<PathBuf>>>,
) {
    if let Some(progress) = parse_progress(line) {
        update_job(app, job_id, |job| {
            job.progress = progress.clone();
            job.message = Some("Descargando…".to_owned());
        });
        emit_job(
            app,
            "download://progress",
        ...
        update_job(app, job_id, |job| {
            ...
            job.message = Some("Fusionando video + audio…".to_owned());
        });
        emit_job(
            app,
            "download://processing",
            ...
            true,
            None,
            Some("Fusionando video + audio…".to_owned()),
            None,
        );
    } else if let Some(json_path) = line.strip_prefix(FILE_PREFIX) {
        if let Ok(path) = serde_json::from_str::<String>(json_path.trim()) {
            if let Ok(mut target) = final_path.lock() {
    ...
    thread::spawn(move || {
        for line in BufReader::new(stream).lines().map_while(Result::ok) {
            ...
            if !line.starts_with(PROGRESS_PREFIX)
                && !line.starts_with(PROCESSING_PREFIX)
                && !line.starts_with(FILE_PREFIX)
            {
                if let Ok(mut log) = diagnostics.lock() {
                    let remaining = DIAGNOSTIC_LIMIT_BYTES.saturating_sub(log.len());
                    if remaining > 0 {
                        let mut consumed = 0usize;
                        log.extend(line.chars().take_while(|character| {
                        ...
                        if log.len() < DIAGNOSTIC_LIMIT_BYTES {
                            log.push('\n');
                        }
                    }
                }
...
fn output_directory(job: &DownloadJob) -> Result<PathBuf, String> {
    ...
    if !directory.is_absolute() {
        return Err("La carpeta de destino debe ser absoluta.".to_owned());
    }
    fs::create_dir_all(&directory)
        .map_err(|error| format!("No se pudo crear la carpeta de destino: {error}"))?;
    directory
        .canonicalize()
        .map_err(|error| format!("No se pudo acceder a la carpeta de destino: {error}"))
}
fn requested_format_selector(
    selected_format_id: Option<&str>,
    selected_format_has_audio: Option<bool>,
    quality_height: Option<u32>,
    compatibility_mode: bool,
) -> String {
    if let Some(format_id) = selected_format_id {
        return if selected_format_has_audio == Some(true) {
            format_id.to_owned()
        } else {
            format!("{format_id}+ba")
        };
    ...
    if compatibility_mode {
        return "bv*+ba/b".to_owned();
    }
    match quality_height {
        Some(height) => format!("bv*[height={height}]+ba"),
        None => "bv*+ba/b".to_owned(),
    }
}
fn format_selector(
    job: &DownloadJob,
    ...
    requested_format_selector(
        selected_format_id,
        selected_format_has_audio,
        job.request.quality_height,
        job.request.compatibility_mode,
    )
}
...
fn download_engine_version(binary: &Path) -> String {
...
fn run_download_attempt(
    ...
    let result = (|| {
        let binary = paths
            .yt_dlp
            .as_ref()
            .ok_or_else(|| DownloadAttemptFailure {
                class: YoutubeFailureClass::Other,
                message: "yt-dlp no está disponible.".to_owned(),
            })?;
        let ffmpeg_directory = engine::yt_dlp_ffmpeg_location(app, paths).map_err(|message| {
            DownloadAttemptFailure {
                class: YoutubeFailureClass::Other,
                message,
            }
        })?;
        let deno = paths.deno.as_ref().ok_or_else(|| DownloadAttemptFailure {
            class: YoutubeFailureClass::Other,
            message: "Deno no está disponible.".to_owned(),
        })?;
        let pot_provider = if strategy.uses_pot_provider() {
        ...
        command
            ...
            .arg(destination)
            .arg("--output")
            .arg("%(title)s [%(id)s] [%(height)sp].%(ext)s")
            .arg("--ffmpeg-location")
        ...
        command
            .arg("--format").arg(format_selector(job, selected_format_id, selected_format_has_audio))
        ...
        if job.request.container != "auto" {
            command
                .arg("--merge-output-format")
                .arg(&job.request.container);
        }
        let mut child = command.spawn().map_err(|_| DownloadAttemptFailure {
            class: YoutubeFailureClass::Other,
            message: "No se pudo iniciar yt-dlp.".to_owned(),
        })?;
        let process_id = child.id();
        let stdout = child.stdout.take().ok_or_else(|| DownloadAttemptFailure {
            class: YoutubeFailureClass::Other,
            message: "No se pudo leer el progreso de yt-dlp.".to_owned(),
        })?;
        let stderr = child.stderr.take().ok_or_else(|| DownloadAttemptFailure {
            class: YoutubeFailureClass::Other,
            message: "No se pudo leer los errores de yt-dlp.".to_owned(),
        })?;
        app.state::<DownloadManager>()
            .processes
            .lock()
            .map_err(|_| DownloadAttemptFailure {
                class: YoutubeFailureClass::Other,
                message: "No se pudo registrar el proceso de descarga.".to_owned(),
            })?
            .insert(job.job_id.clone(), process_id);
        ...
        let exit = child.wait().map_err(|_| DownloadAttemptFailure {
            class: YoutubeFailureClass::Other,
            message: "yt-dlp terminó inesperadamente.".to_owned(),
        })?;
        ...
        app.state::<DownloadManager>()
            ...
            .lock()
            .ok()
            .and_then(|mut processes| processes.remove(&job.job_id));
        if find_job(app, &job.job_id)
            .is_some_and(|current| current.status == DownloadStatus::Cancelled)
        {
            return Err(DownloadAttemptFailure {
                class: YoutubeFailureClass::Other,
                message: "Descarga cancelada.".to_owned(),
            });
        }
        if !exit.success() {
            let diagnostic = diagnostics
                ...
                .unwrap_or_default();
            let class = classify_youtube_failure(&diagnostic);
            let message = youtube_failure_message(&diagnostic, strategy)
                .unwrap_or_else(|| "yt-dlp no pudo completar la descarga.".to_owned());
            return Err(DownloadAttemptFailure { class, message });
        }
        final_path
            .lock()
            .ok()
    ...
    engine::diagnostic_log(
        app,
        &format!(
            "component=download strategy={} duration_ms={} outcome={} engine={engine_version}",
...
fn refreshable_download_failure(class: YoutubeFailureClass) -> bool {
    matches!(
        class,
...
fn execute_download(app: &AppHandle, job: &DownloadJob) -> Result<(), String> {
    ...
    let engine_version = download_engine_version(binary);
    let mut strategy = job.request.effective_access_strategy()?;
    let mut selected_format_id = job.request.selected_format_id.clone();
    let mut selected_format_has_audio = job.request.selected_format_has_audio;
    ...
    let file = loop {
        match run_download_attempt(
            ...
            Err(failure) if !refreshed && refreshable_download_failure(failure.class) => {
                ...
                selected_format_has_audio = resolved.format_has_audio;
                refreshed = true;
                update_job(app, &job.job_id, |current| {
                    current.request.set_access_strategy(strategy.clone());
                    current.request.selected_format_id = selected_format_id.clone();
                    current.request.selected_format_has_audio = selected_format_has_audio;
                    current.message =
    ...
    let canonical_file = file
        .canonicalize()
        .map_err(|error| format!("No se pudo localizar el archivo final: {error}"))?;
    if !canonical_file.starts_with(&destination) {
        return Err("La ruta final no pertenece a la carpeta seleccionada.".to_owned());
    }
    let verification = verify_file(app, &canonical_file)?;
    if verification.video_codec.is_none() || verification.audio_codec.is_none() {
        return Err("El archivo final no contiene video y audio válidos.".to_owned());
    }
    if let Some(height) = job.request.quality_height {
        if verification.height != Some(height) {
            return Err(format!(
                "La verificación final informó {}p, no la calidad solicitada de {height}p.",
                verification.height.unwrap_or(0)
            ));
        }
    }
    ...
    update_job(app, &job.job_id, |current| {
        ...
        current.message = Some("Completado".to_owned());
        current.file_path = Some(file_string.clone());
        current.verification = Some(verification.clone());
        current.completed_at = Some(now_string());
        current.progress.percent = Some(100.0);
    });
    let completed = find_job(app, &job.job_id)
        .ok_or_else(|| "No se encontró el trabajo completado.".to_owned())?;
    if let Err(error) = history::insert(
        app,
        &history::HistoryEntry {
            id: completed.job_id.clone(),
            video_id: completed.request.video_id.clone(),
            url: completed.request.url.clone(),
            title: completed.request.title.clone(),
            thumbnail: completed.request.thumbnail.clone(),
            channel: completed.request.channel.clone(),
            resolution: completed
                .request
                .quality_height
                .map(|height| format!("{height}p"))
                .unwrap_or_else(|| "Mejor calidad".to_owned()),
            container: completed.request.container.clone(),
            file_path: file_string,
            downloaded_at: now_epoch(),
        },
    ) {
        update_job(app, &job.job_id, |current| {
            current.message = Some(format!("Completado; historial no guardado: {error}"))
        });
    }
    emit_job(
        app,
...
fn verify_file(app: &AppHandle, file: &Path) -> Result<DownloadVerification, String> {
    let ffprobe = engine::EnginePaths::resolve(app)
        .ffprobe
    ...
    if !output.status.success() {
        return Err("ffprobe no pudo validar el archivo final.".to_owned());
    }
    let data: Value = serde_json::from_slice(&output.stdout)
        .map_err(|_| "ffprobe devolvió datos inválidos.".to_owned())?;
    let streams = data
        .get("streams")
        .and_then(Value::as_array)
        .ok_or_else(|| "ffprobe no encontró streams.".to_owned())?;
    let video = streams
        .iter()
...
pub fn cancel(app: AppHandle, job_id: String) -> Result<(), String> {
    let process = app
        ...
        .processes
        .lock()
        .map_err(|_| "No se pudo acceder al proceso.".to_owned())?
        .get(&job_id)
        .cloned();
    let job = update_job(&app, &job_id, |current| {
        if !matches!(current.status, DownloadStatus::Completed) {
            current.status = DownloadStatus::Cancelled;
            current.message = Some("Cancelado".to_owned());
        }
    })
    .ok_or_else(|| "No se encontró la descarga.".to_owned())?;
    ...
    emit_job(
        ...
        true,
        None,
        Some("Cancelado".to_owned()),
        None,
    );
    Ok(())
}
pub fn cancel_all(app: AppHandle) -> Result<(), String> {
    let ids: Vec<String> = snapshot(&app)
        .jobs
        .into_iter()
        .filter(|job| {
            matches!(
...
fn safe_job_path(app: &AppHandle, job_id: &str) -> Result<PathBuf, String> {
    if let Some(job) = find_job(app, job_id) {
        let path = job
            .file_path
            .ok_or_else(|| "La descarga aún no tiene un archivo final.".to_owned())?;
        let canonical = PathBuf::from(path)
            .canonicalize()
            .map_err(|_| "El archivo descargado ya no existe.".to_owned())?;
        let destination = PathBuf::from(job.request.destination)
            .canonicalize()
            .map_err(|_| "La carpeta de destino ya no existe.".to_owned())?;
        if !canonical.starts_with(destination) {
            return Err("La ruta del archivo no es válida.".to_owned());
        }
        return Ok(canonical);
    }
    ...
    let entry =
        history::get(app, job_id)?.ok_or_else(|| "No se encontró la descarga.".to_owned())?;
    let path = PathBuf::from(entry.file_path);
    if !path.is_absolute() {
        return Err("La ruta guardada no es válida.".to_owned());
    }
    let canonical = path
        .canonicalize()
        .map_err(|_| "El archivo descargado ya no existe.".to_owned())?;
    if !canonical.is_file() {
        return Err("La ruta guardada no apunta a un archivo.".to_owned());
    }
    Ok(canonical)
}
pub fn open_file(app: AppHandle, job_id: String) -> Result<(), String> {
    let path = safe_job_path(&app, &job_id)?;
    app.opener()
        .open_path(path.display().to_string(), None::<&str>)
        .map_err(|error| format!("No se pudo abrir el archivo: {error}"))
}
pub fn open_folder(app: AppHandle, job_id: String) -> Result<(), String> {
    let path = safe_job_path(&app, &job_id)?;
    app.opener()
...
mod tests {
    ...
    fn selected_format_never_falls_back_to_a_lower_resolution() {
        assert_eq!(
            requested_format_selector(Some("401"), Some(false), Some(2160), false),
            "401+ba"
        );
        assert_eq!(
            requested_format_selector(Some("22"), Some(true), Some(720), false),
            "22"
        );
    }
    ...
    fn compatibility_mode_is_explicit_and_never_used_for_a_source_selection() {
        assert_eq!(
            requested_format_selector(None, None, None, true),
            "bv*+ba/b"
        );
        assert_eq!(
            requested_format_selector(Some("401"), Some(false), Some(2160), true),
            "401+ba"
        );
    }
    ...
    fn accepts_only_supported_local_browser_sessions() {
        ...
        assert!(super::valid_browser_session(Some("brave")));
        assert!(!super::valid_browser_session(Some("safari")));
    }
}
